...
const SECRET_SHAPES: ReadonlyArray<{ name: string; pattern: RegExp }> = [
  ...
  { name: "OpenAI project API key", pattern: /^sk-proj-[\w-]{30,}$/ },
  { name: "Anthropic API key", pattern: /^sk-ant-[\w-]{30,}$/ },
];
...
function createLiteralChecker(
  context: RuleContext,
  handledLiterals: WeakSet<TSESTree.Node>,
): (field: string, valueNode: TSESTree.Node, reportNode: TSESTree.Node) => void {
  return (field, valueNode, reportNode) => {
    if (
...
export default createRule<Options, MessageIds>({
  ...
  create(context) {
    ...
    const checkLiteralFor = createLiteralChecker(context, handledLiterals);
