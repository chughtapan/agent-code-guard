...
export const SEEDS: readonly RuleSeed[] = [
  ...
  { ruleId: "agent-code-guard/prefer-annotate-logs", seed: 'Effect.log("loaded", { id: 1 });', coFire: [] },
  { ruleId: "agent-code-guard/annotate-without-span", seed: "Effect.annotateCurrentSpan({ x: 1 });", coFire: [] },
  { ruleId: "agent-code-guard/prefer-config-redacted", seed: 'const apiKey = Config.string("API_KEY");', coFire: [] },
  { ruleId: "agent-code-guard/effect-foreach-requires-concurrency", seed: "Effect.forEach(items, fn);", coFire: [] },
  { ruleId: "agent-code-guard/acquire-release-requires-scope", seed: "const program = Effect.acquireRelease(open, close);", coFire: [] },
  { ruleId: "agent-code-guard/finalizer-requires-scope", seed: "Scope.addFinalizer(close);", coFire: [] },
  { ruleId: "agent-code-guard/parse-into-schema-requires-effect", seed: "const u = Schema.decodeUnknownSync(S)(JSON.parse(input));", coFire: ["agent-code-guard/prefer-decode-effect-at-boundary"] },
...
const IDENTS_BY_SEED: Record<string, string> = {
  ...
  "agent-code-guard/prefer-config-redacted": "apiKey",
  "agent-code-guard/effect-foreach-requires-concurrency": "items",
  "agent-code-guard/acquire-release-requires-scope": "open",
  "agent-code-guard/finalizer-requires-scope": "close",
  "agent-code-guard/parse-into-schema-requires-effect": "input",
