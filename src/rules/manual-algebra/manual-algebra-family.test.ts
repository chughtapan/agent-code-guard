...
it("Property 6: manual-brand still fires on branded surfaces under rename and whitespace mutation", () => {
  const seed = 'type UserId = string & { readonly __brand: "UserId" };';
  fc.assert(
    fc.property(paddingArb, renameArb, (pad, rename) => {
      const code = mutate(seed, pad, "UserId", rename);
...
it("Property 6b: no-manual-brand-constructor still fires under mutation", () => {
  const seed = "const asUserId = (value: string): UserId => value as UserId;";
  fc.assert(
    fc.property(paddingArb, renameArb, (pad, rename) => {
      const code = mutate(seed, pad, "value", rename);
      expect(lintOne(code, "agent-code-guard/no-manual-brand-constructor")).toHaveLength(1);
    }),
    { numRuns: 30 },
  );
});

it("Property 7: manual-result still fires on helper functions under mutation", () => {
...
it("Property 8: manual-option still fires on helper functions under mutation", () => {
